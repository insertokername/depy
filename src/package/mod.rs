...
impl Package {
    ...
    pub fn single_package_from_json(
        ...
        let name = name_field.as_str().ok_or_else(||{
    ...
    pub fn multiple_packages_from_json(
        ...
        if let Some(out) = json.as_array() {
            let temp = out
                .into_iter()
                .map(|pkg| Package::single_package_from_json(pkg).map_err(|e| e))
    ...
    pub fn save_packages_to_json(
        ...
        std::fs::write("./depy.json", str_package).map_err(|err|PackageError::PacakgeSaveError(err.to_string()))?;
        Ok(())
    }
}
